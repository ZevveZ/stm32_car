...
static float sonar_trigger(void){
	...
	while(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_9)==RESET&&delay_get_timing_delay());	//在SONAR_MAX_WAIT_TIME时间内等待是否有回声
	
	if(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_9)){	//接受到回声
		TIM6->CNT=0;
		TIM_Cmd(TIM6,ENABLE);	//开始计时
		while(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_9)==SET);	//轮询
